...
class Morse(Wavelet):
    ...
    def time_domain(self, t):
        ...
        n = m // 8
        f0 = 1.0 / (8 * dt)
        c = [self.freq_domain(2 * f0 * l / float(n)) for l in range(n // 2 + 1)]
        x = np.zeros(len(t), dtype='c16')

        # lilly2009higher equation (?)
        for i in range(len(t)):
            for l in range(n // 2):
                x[i] += c[l] * np.exp(1j * 2.0 * np.pi * l * i / m)
            x[i] /= (m * dt)
...
def cwt(x: np.ndarray, dt: float, scales: np.ndarray, mother: Wavelet,
    ...
    """
    Continuous wavelet transform with synchrosqueezing. To plot the coi_time_idxs, use plot(t[coi_time_idxs], freqs[-len(coi_time_idxs):]).
    ...
    :param syncsqz_freqs: destination frequency bin centers for synchrosqueezing, None to skip synchrosqueezing
    ...
    :return: returns an array of shape (len(scales), len(x)), coi_mask of same size, and coi_time_idxs time-indexes per scale, and coi_freq_idxs of remaining freqs
    """
