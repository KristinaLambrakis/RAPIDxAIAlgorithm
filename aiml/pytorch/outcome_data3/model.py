...
class TroponinNet(nn.Module):
    ...
    def network(self):
        ...
        input_len = self.feature_len['luke'] * self.luke_multiplier + self.feature_len['phys'] + \
                    self.feature_len['bio'] + self.feature_len['bin']
    ...
    def __init__(self, target_info):
        ...
        self.feature_len = protocol.get_feature_len(data_cohort=target_info['data_cohort'], use_ecg=target_info['use_ecg'])
    ...
    def forward(self, input_feature):
        ...
        input_dict = {k: input_feature[:, v[0]:v[1]] for k, v in self.feature_arrangement.items()}

        feature_names = ['luke'] * self.luke_multiplier + ['phys', 'bio', 'bin']
        input_feature = torch.cat([input_dict[k] for k in feature_names], dim=1)
        input_feature = input_feature.view(input_feature.size()[0], -1)
        features = self.net(input_feature)
...
class TroponinShell(nn.Module):
    def __init__(self, model):
        super(TroponinShell, self).__init__()
        self.model = model

    def forward(self, input_feature):
        output = self.model(input_feature)
        return torch.softmax(output[2][0], dim=1)
