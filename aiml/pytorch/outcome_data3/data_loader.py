...
import aiml.pytorch.outcome_data3.protocol as protocol
...
class TroponinDataset(data.Dataset):
    ...
    def __init__(self, csv_path,
                 ...
                 use_random_crop=None, transform=None, prefill_feature=None, data_cohort='f'):
        ...
        self.prefill_feature = prefill_feature
        self.data_cohort = data_cohort

        if self.prefill_feature:
            self.df = prefiller(self.df, prefill_value_dict=config['prefill']['all'])
        ...
        self.luke_trop_keys = protocol.get_luke_trop_keys(data_cohort=self.data_cohort)
        self.luke_mean = torch.tensor(np.array(self.stats[self.luke_trop_keys].loc['mean']), dtype=torch.float).reshape(
            -1, 1,
            1)
        self.luke_std = torch.tensor(np.array(self.stats[self.luke_trop_keys].loc['std']), dtype=torch.float).reshape(
            -1, 1, 1)
        ...
        self.phys_keys = protocol.get_phys_keys(data_cohort=self.data_cohort)
        self.phys_mean = torch.tensor(np.array(self.stats[self.phys_keys].loc['mean']), dtype=torch.float).reshape(-1,
                                                                                                                   1, 1)
        self.phys_std = torch.tensor(np.array(self.stats[self.phys_keys].loc['std']), dtype=torch.float).reshape(-1, 1,
                                                                                                                 1)

        self.bio_keys = protocol.get_bio_keys(data_cohort=self.data_cohort)
        self.bio_mean = torch.tensor(np.array(self.stats[self.bio_keys].loc['mean']), dtype=torch.float).reshape(-1, 1,
                                                                                                                 1)
        self.bio_std = torch.tensor(np.array(self.stats[self.bio_keys].loc['std']), dtype=torch.float).reshape(-1, 1, 1)

        self.binary_keys = protocol.get_binary_keys(data_cohort=self.data_cohort, use_ecg=self.target_info['use_ecg'])
    ...
    def get_item(self, info):
        ...
        feature = self.get_feature(info)
        targets = self.get_targets(info)
    ...
    def get_feature(self, info):
        ...
        features_bin = torch.tensor(np.array(np.array([info[k] for k in self.binary_keys])).reshape(-1, 1, 1),
                                    dtype=torch.float)
        selector = features_bin == self.ignore_value
        features_bin[selector] = 0
        ...
        # raw trop: 6, time trop: 6, phys: 19, bio: 2, bin: 24, luke: 10
        feature = torch.cat([  # feature_raw_trop, time_raw_trop,
            features_phys, features_bio, features_bin, feature_luke], dim=0)
...
def get_loader_from_dataset(csv_path, target_info, target_translator, batch_size, transform, set_key, set_scope,
                            use_random_crop, prefill_feature, data_cohort, shuffle, num_workers, drop_last):
