import argparse
import os
from troponin.src import utils
from troponin.src.lsw import stats as lsw_stats
from path_utils import cache_root
import numpy as np
import pandas as pd
import aiml.data.raw_data as data
...
def get_features(max_ntrop):
    ...
    d = data.get_data()
    d.to_csv(os.path.join(cache_root, 'data.csv'))

    # output_path = 'cache/feature_counts2/'
    # os.makedirs(output_path, exist_ok=True)
    #
    # for _, v in data.get_short_test_label_map().items():
    #     sub_d = d['phys_{}'.format(v)]
    #
    ...
    #     plt.xlabel('Bins')
    #     plt.ylabel('Counts')
    #     plt.savefig(os.path.join(output_path, '{}.svg'.format(v)), format="svg")
    ...
    phys_labels = [f'phys_{label}' for label in list(data.get_short_test_label_map().values())]
    ...
    risk_factors = ['smoking', 'gender', 'age', 'hst_priormi', 'hst_dm', 'hst_htn', 'hst_std', 'ischaemia_ecg',
                    'cad', 'dyslipid', 'fhx', 'hst_familyhx', 'hr', 'sbp', 'dbp', 'mdrd_gfr',
                    'onset', 'hst_onsetrest', 'hst_angio', 'angiogram']
    ...
    df_ids = df[['cohort_id', 'ds', 'supercell_id', 'subjectid', 'rapid_group']]
    ...
    df_features.loc[df_features['hst_onsetrest'] == 9, 'hst_onsetrest'] = np.nan
...
def main(args):
    ...
    for trop_idx in range(6):
        ...
        locator = (time_trop >= time_gates_start) & (time_trop < time_gates_end)
    ...
    # matplotlib.use('TkAgg')
    #
    # feat = np.array(df_features[['trop{}'.format(i) for i in range(6)]])
    # time = np.array(df_features[['time_trop{}'.format(i) for i in range(6)]])
    # mat = np.stack([time, feat], axis=2)
    # num_days = 7
    # # mat = np.concatenate([mat, np.repeat(np.array([[[24*num_days, 0]]]), mat.shape[0], axis=0)], axis=1)
    # # num_days = 7
    # # mat = np.concatenate([mat, np.repeat(np.array([[[24 * num_days, 0]]]), mat.shape[0], axis=0)], axis=1)
    # ps = [curve_fit(func, row[~np.isnan(row[:, 0]), 0], row[~np.isnan(row[:, 0]), 1],
    #                 bounds=([-np.inf, 0, 0, 0], [np.inf, np.inf, np.inf, np.inf]))[0] for row in mat[:10, ]]
    # xp = np.linspace(0, 24 * num_days, 24 * num_days * 5)
    #
    # for i in range(10):
    #     x, y = mat[i, :, 0], mat[i, :, 1]
    #     base = x[0]
    #     plt.plot(x, y/base, '.', xp, func(xp, *ps[i])/base, '-')
    # plt.show()

    stats = dict()
    for k in df_features.keys():
        print(k)
        if k not in ['out5', 'out3c', 'outl1', 'outl2', 'cohort_id', 'ds', 'supercell_id', 'subjectid', 'rapid_group']:
    ...
    stats.to_csv(os.path.join(cache_root, 'data_raw_trop{}_phys_stats.csv'.format(args.num_troponins)))
    ...
    df_master['set'] = 'train'
    df_master.to_csv(os.path.join(cache_root, 'data_raw_trop{}_phys.csv'.format(args.num_troponins)))
    ...
    for idx in range(args.num_boots):
        ...
        # df_features_angio_diff = df_features.loc[(df_features['hst_angio'] != df_features['angiogram']) & (pd.notna(df_features['hst_angio']))]
        # df_features_angio_diff.to_csv(os.path.join(cache_root, 'data_raw_trop{}_phys_angio_diff.csv'.format(args.num_troponins)), index=False)
        ...
        np.save(os.path.join(cache_root, 'data_raw_trop{}_phys_{}_onehot_encoding.npy'.format(args.num_troponins, idx)), onehot_choices)
