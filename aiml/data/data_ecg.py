...
import numpy as np
import pandas as pd
import aiml.data.raw_data_ecg as data
...
def get_features(max_ntrop):
    d = data.get_data_ecg()
    d.to_csv(os.path.join(cache_root, 'data_ecg.csv'))
    ...
    risk_factors = configs['features']['prior']['data_ecg'] + configs['features']['ecg']['data_ecg'] + \
...
def make_drawings(df_features):
    ...
    for k in df_features.keys():
        if k in configs['features']['phys']['data2'] + configs['features']['prior']['data_ecg'] + \
                configs['features']['ecg']['data_ecg']:
            plt.clf()
    ...
    plt.savefig(os.path.join(cache_root, 'data_ecg_abs_corr.svg'))


def main(args):
    df_features = get_features(args.num_troponins)

    df_features = df_features.drop(columns=configs['features']['exclude']['data_ecg'])
    ...
    for k in df_features.keys():
        print(k)
        if k not in ['idPatient', 'adjudicatorDiagnosis']:
    ...
    for idx in range(args.num_boots):
        ...
        idx_list = list(range(len(df_features)))
        ...
        assert sum([len(df_features.loc[df_features['set{}'.format(idx)] == v]) for v in ['train', 'val', 'test']]) == len(
            df_features)
