import os
import pickle
from validator_outcome_xgb_data3 import load_model
...
def main(args):
    ...
    models_all = [([None, dict()], [None, dict()]) for i in range(50)]
    for threshold_method in ['roc', 'tpr', 'default', 'pr']:
        args.threshold_method = threshold_method
        models, df_outbags = load_model(args)
        for m_idx, ((m1, t1), (m2, t2)) in enumerate(models):
            if models_all[m_idx][0][0] is None:
                models_all[m_idx][0][0] = m1
            if models_all[m_idx][1][0] is None:
                models_all[m_idx][1][0] = m2
            models_all[m_idx][0][1][threshold_method] = t1
            models_all[m_idx][1][1][threshold_method] = t2
    for m_idx, m in enumerate(models_all):
        models_all[m_idx] = (tuple(m[0]), tuple(m[1]))
    ...
    dump_path = os.path.join(dump_path, 'outcome_models_xgb_data3.pickle')
